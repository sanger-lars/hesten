...
function clikket(e) {
  ...
  mc2.forEach(function (delt) {
    return delt.addEventListener('mousedown', function () {
      console.log(arr_id);
      var deltager_navn = document.getElementById('navn').value;
      var deltager_antal = document.getElementById('antal').value;
      if (deltager_navn == "") {
...
function slet_arangement(e) {
  ...
  if (nr !== "false") {
    ...
    svar_ja = false;
    c = this.children;
    var svar_ja = confirm("vil du slette denne begivenhed ? id= "+id);
    if (svar_ja) {
      // find billedet
...
/*  var delta = document.querySelectorAll('#deltag');
  delta.forEach(function (delt) {
    return delt.addEventListener('mousedown', clikket);
  });*/
}

function klargor_vis_deltager_klik() {
  /*$("#dtekst").each(function () {   
    $(this).on("click", vis_deltagere);
  })
  /*var vis = document.querySelectorAll('#dtekst');
  vis.forEach(function (delt) {
    return delt.addEventListener('mousedown', vis_deltagere);
  });*/
}
...
function hent_tal_fra_DB(fra, callback) {
  var posting = $.post("includes/hent.php", {
    ...
    if (!data == "") {
      ...
      if (slet) {
        $(".arangement").each(function () {
          $(this).on("click", slet_arangement);
        })
        //var delta = document.querySelectorAll('.arangement');
        //delta.forEach(function (delt) {
          // return delt.addEventListener('mousedown', slet_arangement);
         // return delt.
       // });
