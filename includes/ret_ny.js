...
if (xxx > "") {
	...
	if (zzz[1] !== "") {
		...
		if (zzz[0].substring(0, 3) !== "img") {
			...
			// document.getElementById('opret_submit').classList.toggle('--gul_knap');
			nr = parseInt(zzz[2]);
		}
	}
	pass = zzz[0];
...
$("#opdater").on("mousedown", function (e) {
	...
	if (dato !== arr_dato) {
		...
		var arr_html = '<div id="' + arr_id + '" class="arangement">' + '<div><h2>Dato: ' + arr_date + '</h2></div>' + '<div class="overskrift"><h2>' + arr_overskrift + '</h2></div>';
		if (billede == "true") {
			arr_html = arr_html + '<img src="uploads/' + arr_img + '">';
		}
		arr_html = arr_html + '<div id="tekst">' + arr_tekst + '</div>' + '<div id="deltagere"><span id="dtekst">Deltagere:' + arr_deltagere + '</span> <a id ="deltag">Deltag</a> </div>' + '</div>';
...
var posting = $.post("hent.php", {
	...
	// 1966-11-30
	// 0123456789
	arr_date = arr_dato.substring(8, 10) + "-" + arr_dato.substring(5, 8) + arr_dato.substring(0, 4);
